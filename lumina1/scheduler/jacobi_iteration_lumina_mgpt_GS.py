        ...
        for b in range(batch_size):
            for s in range(seq_len):
                ...
                token_id = _sample_from_probs_with_global_rand(
                    probs, random_table, batch_idx=b, step_idx=step_idx, vocab_size=vocab_size
                )
                next_tokens[b, s] = token_id
    ...
    class JacobiSampler(model_class, nn.Module):
        ...
        def _extract_past_from_model_output(self, outputs: ModelOutput):
            for possible_cache_name in ("past_key_values", "mems", "past_buckets_states", "cache_params"):
                if possible_cache_name in outputs:
                    cache_name = (
                        "past_key_values"
                        if possible_cache_name in ("mems", "past_buckets_states")
                        else possible_cache_name
                    )
                    return cache_name, getattr(outputs, possible_cache_name)
        ...
        def _sample(
            ...
            **model_kwargs,
            ...
            self.random_table = torch.rand([1, 64, 65536], device=device)
            count_time = True
            if count_time:
            ...
            while self._has_unfinished_sequences(
                ...
                next_tokens, next_token_scores = sampling_logits2tokens(
                    ...
                    random_table = self.random_table
                )
                ...
                self.current_accepted_length = num_matched_tokens
                if not (output_token_num <= 1):
                    self.random_table[:,:-num_matched_tokens,:] = self.random_table[:,num_matched_tokens:,:].clone()
                    self.random_table[:,64-num_matched_tokens:,:] = torch.rand([1,num_matched_tokens,65536], device=device)
