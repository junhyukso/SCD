...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.tokens_per_row = 49 #yes this is correct ..

        self.img_start_token = torch.tensor([8197], device='cuda')
        ...
        #self.tokens_per_row = torch.zeros((self.tokens_per_row-1,), dtype=torch.long)
        #self.unconverged_tokens_per_row = torch.zeros((self.tokens_per_row-1,), dtype=torch.long)
        #self.row_res = [None for i in range(self.tokens_per_row-1)]
