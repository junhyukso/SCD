...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.attn_type == "self":
            ...
            # The total length of keys/values to attend to is the length of the mask's last dimension
            total_kv_len = mask.shape[-1]
            keys = self.cache_k[:bsz, :, :total_kv_len]
            values = self.cache_v[:bsz, :, :total_kv_len]
        else:
            keys, values = xk, xv
