...
def sample_top_p_all_positions(logits: torch.Tensor, temperature: float, top_p: float, return_probs: bool = False):
    """
    Vectorized top-p (nucleus) sampling over all positions.

    Args:
        logits (torch.Tensor): Tensor of shape [batch, length, vocab].
        temperature (float): Sampling temperature.
        top_p (float): Nucleus threshold.
        return_probs (bool): If True, return renormalized probabilities in original index order with shape [B, L, V].

    Returns:
        Tuple[torch.Tensor, Optional[torch.Tensor]]: Sampled token indices of shape [B, L] and optional probs [B, L, V].
    """
    assert logits.ndim == 3, f"Expected logits of shape [B, L, V], got {list(logits.shape)}"
    # Stabilize temperature
    probs = torch.softmax(logits / max(temperature, 1e-5), dim=-1)
    # Sort probabilities and get indices
    probs_sort, probs_idx = torch.sort(probs, dim=-1, descending=True)
    # Compute cumulative sums along vocab dimension
    probs_sum = torch.cumsum(probs_sort, dim=-1)
    ...
    next_token_sorted_idx = multinomial_sample_one_no_sync(probs_sort, dtype=torch.int64)  # [B, L, 1]
    next_token = torch.gather(probs_idx, -1, next_token_sorted_idx).squeeze(-1)  # [B, L]
    if return_probs:
        probs_unsorted = torch.zeros_like(probs_sort)
        probs_unsorted.scatter_(-1, probs_idx, probs_sort)
    else:
